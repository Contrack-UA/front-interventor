...
class App extends Component {
    ...
    render() {
        ...
        if (status === "log") {
            ...
            if (status === "my") {
                element2 = <Recipes username={this.state.username} password={this.state.password} selected={this.changeStat.bind(this)}/>;
            }
            if (status==="srch"){
              element2 = (<div><div className="row"><Well></Well></div>
              <div className="row"></div><br/><h1>La factura está siendo procesada y será publicada en unos segundos</h1></div>);
            }
            if (status==="add"){
